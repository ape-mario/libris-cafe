...
export interface ReadingStats {
  ...
  ratingDistribution: number[]; // index 0-4 = counts for 1-5 stars
...
export async function getReadingStats(userId: string): Promise<ReadingStats> {
  const [allBooks, userData] = await Promise.all([
    db.books.toArray(),
    db.userBookData.where('userId').equals(userId).toArray()
  ]);

  const bookMap = new Map(allBooks.map(b => [b.id, b]));

  // Only consider books this user has interacted with
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    // Count user's books added in this month that are read
    const count = userBooks.filter(book => {
      const added = new Date(book.dateAdded);
      const sameMonth = added.getFullYear() === d.getFullYear() && added.getMonth() === d.getMonth();
  ...
  // Genre breakdown from user's books only
  ...
  // Top authors from user's books only
