...
export async function getRecommendations(limit: number = 6): Promise<Recommendation[]> {
  ...
  for (const seed of seedBooks) {
    ...
    try {
      // Search by the book's primary category or author
      const query = seed.categories[0]
        ? `subject:${seed.categories[0]}`
        : `author:${seed.authors[0] || ''}`;

      const res = await fetch(
        `https://openlibrary.org/search.json?q=${encodeURIComponent(query)}&limit=20&fields=title,author_name,isbn,cover_i`
      );
      if (!res.ok) continue;
