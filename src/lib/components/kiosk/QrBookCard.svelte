...
<div class="bg-warm-50 rounded-xl overflow-hidden shadow-sm active:scale-[0.98] transition-transform">
  <!-- Cover image -->
  <div class="aspect-[2/3] bg-warm-100 relative">
    {#if book.coverUrl}
      <img
        src={book.coverUrl}
        alt={book.title}
    ...
    {#if qrUrl}
      <img
        src={qrUrl}
        alt="QR"
        class="absolute bottom-1 right-1 w-12 h-12 bg-surface rounded p-0.5"
      />
    {/if}
  </div>
  ...
  <div class="p-2">
    ...
    {#if book.authors?.length}
      <p class="text-xs text-ink-muted mt-0.5 truncate">{book.authors.join(', ')}</p>
    {/if}
  </div>
</div>
